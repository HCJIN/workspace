...
public class Bank {
    ...
    public Client info(String accNum, String name){
        for (Client ct : clientList){
            if(ct.getAccInfo().getAccNUmber().equals(accNum) && ct.getName().equals(name)){
                return ct;
            }
        }
        return null;
    ...
    //메서드를 정의하시오.
    public List<Client> money(int balance){
        List<Client> cl = new ArrayList<>();
        for (Client ct : clientList){
            if (ct.getAccInfo().getBalance() > balance){
                cl.add(ct);
            }
        }
        return cl;
